...
// BigInt serialization is handled globally by src/lib/bigint-fix.ts
// (patches BigInt.prototype.toJSON so JSON.stringify converts them automatically)
...
export async function GET(req: any) {
  try {
    ...
    try {
      ...
      const [taskResults, countResult] = await Promise.all([
        (prisma.task as any).findMany({
          ...
          take: limit,
          skip: (page - 1) * limit,
          // Sort by title ASC in the DB — titles follow CompanyName_DD-MM-YYYY_TypeNumber
          // format, so alphabetical sort naturally groups by company → date → type
          orderBy: { title: "asc" },
          select: {
            id: true,
            title: true,
    ...
    // ✅ Return paginated tasks (sorted by DB via title ASC)
    return NextResponse.json({
      tasks: tasks,
      pagination: {
        page,
