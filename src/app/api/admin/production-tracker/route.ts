...
export async function GET(req: NextRequest) {
  try {
    ...
    const clientProgress = clients.map((client) => {
      ...
      const deliverables = client.monthlyDeliverables.map((del) => {
        ...
        const statusCounts = {
          ...
          pending: allDelTasks.filter(
            (t) => t.status === "PENDING" || t.status === "VIDEOGRAPHER_ASSIGNED"
          ).length,
          inProgress: allDelTasks.filter((t) => t.status === "IN_PROGRESS").length,
          readyForQc: allDelTasks.filter(
            (t) => t.status === "READY_FOR_QC" || t.status === "QC_IN_PROGRESS"
          ).length,
          completed: allDelTasks.filter((t) => t.status === "COMPLETED").length,
          scheduled: allDelTasks.filter((t) => t.status === "SCHEDULED").length,
        ...
        const doneCount = promisedDelTasks.filter(
          (t) =>
            t.status === "COMPLETED" ||
            t.status === "SCHEDULED" ||
            t.status === "POSTED"
      ...
      const totalPromised = deliverables.reduce((s, d) => s + d.promised, 0);
      const totalDone = deliverables.reduce((s, d) => s + d.doneCount, 0);
      const overallProgress =
        totalPromised > 0 ? Math.round((totalDone / totalPromised) * 100) : 0;
    ...
    const atRiskClients = clientProgress
      .filter(
        (c) => c.totalPromised > 0 && (c.health === "critical" || c.health === "warning")
      )
    ...
    const summary = {
      totalClients: clients.filter((c) => c.monthlyDeliverables.length > 0).length,
...
function buildEditorPerformance(employees: any[], tasks: any[], role: string) {
  const editors = employees.filter((e) => e.role?.toLowerCase() === role);
  ...
  return editors.map((editor) => {
    ...
    const completionRate =
      editorTasks.length > 0
        ? Math.round((statusBreakdown.completed / editorTasks.length) * 100)
        : 0;
    ...
    const clientIds = [
      ...new Set(editorTasks.map((t: any) => t.clientId).filter(Boolean)),
    ];
...
function buildQCPerformance(employees: any[], tasks: any[]) {
  ...
  const qcUsers = employees.filter(
    (e) => e.role?.toLowerCase() === "qc" || qcReviewerIds.includes(e.id)
  );
  ...
  return uniqueQC.map((qcUser) => {
    const reviewedTasks = tasks.filter((t) => t.qcReviewedBy === qcUser.id);
    ...
    if (reviewedWithTime.length > 0) {
      const totalHours = reviewedWithTime.reduce((sum: number, t: any) => {
        const reviewDate = new Date(t.qcReviewedAt);
        const createDate = new Date(t.createdAt);
        return (
          sum +
          (reviewDate.getTime() - createDate.getTime()) / (1000 * 60 * 60)
        );
      }, 0);
      avgReviewHours =
...
function buildSchedulerPerformance(employees: any[], tasks: any[]) {
  ...
  const schedulerUsers = employees.filter(
    (e) =>
      e.role?.toLowerCase() === "scheduler" || schedulerIds.includes(e.id)
  );
  ...
  return uniqueSchedulers.map((sched) => {
    ...
    const posted = schedTasks.filter((t) => t.status === "POSTED").length;
