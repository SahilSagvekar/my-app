...
export async function POST(req: Request) {
  try {
    ...
    if (status === 'completed') {
      ...
      console.log('   📥 Fetching full transcript from AssemblyAI...');
      ...
      const audio_duration = transcriptData.audio_duration;

      if (!text) {
        throw new Error('Transcript text is empty');
      }

      console.log(`   ✅ Transcript fetched, length: ${text.length} chars`);
      ...
      const error = payload.error || 'Unknown transcription error';
      console.error(`   ❌ Transcription failed: ${error}`);
      
      await failTitlingJob(transcript_id, error);
      ...
      console.log(`   ℹ️ Received status: ${status} (waiting for completion)`);
      return NextResponse.json({ success: true, message: `Status ${status} acknowledged` });
    }
