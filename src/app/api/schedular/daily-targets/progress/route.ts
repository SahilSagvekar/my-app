...
function getESTDate(date?: string): { start: Date; end: Date } {
  ...
  // Start and end of day in EST
  const startEST = new Date(estDate);
  startEST.setHours(0, 0, 0, 0);
  const endEST = new Date(estDate);
  endEST.setHours(23, 59, 59, 999);

  // Convert back to UTC for database queries
  // EST is UTC-5, EDT is UTC-4. We use the offset from the original conversion.
  const offsetMs = now.getTime() - estDate.getTime();

  return {
    start: new Date(startEST.getTime() + offsetMs),
    end: new Date(endEST.getTime() + offsetMs),
  };
}
...
function getESTDayOfWeek(date?: string): number {
  const now = date ? new Date(date) : new Date();
  const estString = now.toLocaleString('en-US', { timeZone: 'America/New_York' });
  return new Date(estString).getDay(); // 0=Sunday, 1=Monday, ...
}
...
function getESTWeekBounds(date?: string): { start: Date; end: Date } {
  ...
  monday.setDate(estDate.getDate() + mondayOffset);
  ...
  return {
    start: new Date(monday.getTime() + offsetMs),
    end: new Date(sunday.getTime() + offsetMs),
  };
}
