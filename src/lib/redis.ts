...
export async function cached<T>(
  key: string,
  fn: () => Promise<T>,
  ttl: number = 300 // seconds — was 60; bumped to cut Upstash command volume
): Promise<T> {
  if (!redis) return fn();
...
// NOTE: these no longer call redis.keys() (an expensive scan command that
// burns Upstash free-tier command quota fast). They're now no-ops — stale
// cache entries just expire on their own via the TTL above (max 5 min stale).
// If you need instant invalidation back, call redis.del(key) with the exact
// key(s) you wrote in `cached()` instead of pattern-scanning.
export async function invalidateTaskCache(userId: number) {
  // intentionally no-op — relies on cache TTL expiry, see note above
}
